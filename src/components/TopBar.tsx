...
import { Terminal, FolderGit2, ChevronDown, User, Shield, Sparkles, Rocket, Key } from 'lucide-react';
...
export const TopBar: React.FC<TopBarProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Right Controls: API Key, Command Palette Trigger, Supabase Auth Shield, & Runway Capsule */}
      <div className="flex items-center space-x-2.5 sm:space-x-3">
        {onOpenApiKeyModal && (
          <button
            onClick={onOpenApiKeyModal}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-white/20 text-xs text-[#94A3B8] hover:text-white transition-all cursor-pointer shadow-sm group"
            title="Custom Gemini API Key Settings"
          >
            <Key className="w-3.5 h-3.5 text-[#00F0FF] group-hover:rotate-12 transition-transform" />
            <span className="hidden sm:inline font-mono text-[11px] font-bold tracking-wider">API KEY</span>
            {hasCustomApiKey ? (
              <span className="w-2 h-2 rounded-full bg-[#00FF88] shadow-[0_0_6px_#00FF88]" title="Custom API key active" />
            ) : (
        ...
        {onOpenAuth && (
          <button
            onClick={onOpenAuth}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-white/20 text-xs text-[#94A3B8] hover:text-white transition-all cursor-pointer shadow-sm"
        ...
        <button
          ...
          className="hidden md:flex items-center space-x-2 px-3 py-2 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-white/20 text-xs text-[#94A3B8] hover:text-white transition-all group cursor-pointer shadow-sm"
